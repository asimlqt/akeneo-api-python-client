...
> PAMAssetCategories get_asset_categories(authorization, page=page, limit=limit, with_count=with_count)
...
try:
    # Get list of PAM asset categories
    api_response = api_instance.get_asset_categories(authorization, page=page, limit=limit, with_count=with_count)
    pprint(api_response)
...
> InlineResponse20036 get_asset_categories_code(authorization, code)
...
try:
    # Get a PAM asset category
    api_response = api_instance.get_asset_categories_code(authorization, code)
    pprint(api_response)
except ApiException as e:
